      ...
      {loading ? (
        ...
        <>
          <View style={styles.headerRow}>
            <Text style={styles.headerCell}>{t('pointsTable.header.letters')}</Text>
            <Text style={[styles.headerCell, styles.headerRight]}>{t('pointsTable.header.coins')}</Text>
          </View>
          {rows.length === 0 ? (
            <Text style={styles.empty}>{t('pointsTable.empty')}</Text>
          ) : (
            rows.map((row) => (
              <View key={row.length} style={styles.dataRow}>
                <Text style={styles.dataCell}>{t('pointsTable.row.letters', { n: row.length })}</Text>
